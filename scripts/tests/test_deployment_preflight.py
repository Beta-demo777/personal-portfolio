...
class DeploymentPreflightTests(unittest.TestCase):
    ...
    def test_validates_secret_contract_and_allows_empty_ai_api_key(self) -> None:
        deployment_preflight.validate_env(self.root)

    def test_allows_default_model_and_validates_optional_override(self) -> None:
        ai_api_key = self.paths["PORTFOLIO_AI_API_KEY_SECRET_FILE"]
        ai_api_key.write_text("test-ai-key", encoding="utf-8")
        deployment_preflight.validate_env(self.root)

        self._write_env(replace={"AI_MODEL": "deepseek-v4-pro"})
        deployment_preflight.validate_env(self.root)

        self._write_env(replace={"AI_MODEL": "unsupported/model"})
