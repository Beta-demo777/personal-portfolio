...
export PORTFOLIO_GEMINI_API_KEY_SECRET_FILE=$secret_directory/gemini_api_key
...
assert_secret_projection() {
    "${COMPOSE[@]}" run --rm --no-deps -T --entrypoint /bin/sh secret-init -euc '
        ...
        test -z "${PORTFOLIO_GEMINI_API_KEY_SECRET_FILE+x}"
        ...
        cmp -s /run/secrets/gemini_api_key /runtime/frontend/gemini_api_key
        for target in \
            ...
            /runtime/frontend/gemini_api_key; do
...
write_secret_file "$PORTFOLIO_GEMINI_API_KEY_SECRET_FILE" "$initial_ai_key"
...
cp "$PORTFOLIO_GEMINI_API_KEY_SECRET_FILE" "$retired_secret_directory/gemini_api_key"
...
write_secret_file "$PORTFOLIO_GEMINI_API_KEY_SECRET_FILE" "$(random_secret)"
...
IFS=$'\t' read -r \
    ...
    commit_staged_database \
    commit_rollback_database \
    commit_restore_phase \
    commit_backend_was_active \
