...
class ComposeConfigurationTests(unittest.TestCase):
    ...
    def test_secret_initializer_keeps_only_the_required_host_secret_capability(self) -> None:
        rendered = self._run_config(render=True)
        self.assertEqual(rendered.returncode, 0, rendered.stderr)
        service = json.loads(rendered.stdout)["services"]["secret-init"]
        self.assertEqual(service["cap_add"], ["DAC_READ_SEARCH"])
