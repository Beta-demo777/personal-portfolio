...
import { DEFAULT_AI_MODEL, loadServerConfig } from './config';
...
test('defaults to the current DeepSeek model and validates optional overrides', () => {
  assert.equal(loadServerConfig({}).agent.model, DEFAULT_AI_MODEL);
  assert.equal(loadServerConfig({ AI_MODEL: 'deepseek-v4-pro' }).agent.model, 'deepseek-v4-pro');
  assert.equal(
    loadServerConfig({ AI_API_KEY: 'test-key' }).agent.apiKey,
    'test-key',
  );
  assert.throws(
    () => loadServerConfig({ AI_MODEL: 'unsupported/model' }),
    /AI_MODEL contains unsupported characters/,
  );
});
