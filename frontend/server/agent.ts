...
export interface AgentDependencies {
  now?: () => number;
  createClient?: (apiKey: string, timeoutMs: number) => GoogleGenAI;
...
export function createAgentRouter(
  ...
  const createClient = dependencies.createClient ?? ((apiKey: string, timeoutMs: number) => (
    new GoogleGenAI({
      apiKey,
      httpOptions: {
        timeout: timeoutMs,
        headers: { 'User-Agent': 'beta-demo-portfolio-agent' },
      },
    })
  ));
  ...
  router.post('/api/agent/chat', async (req: Request, res: Response) => {
    ...
    if (!config.apiKey) {
      return res.json({
        message: 'AI 对话服务尚未配置 `GEMINI_API_KEY`。你仍可浏览作品集、项目与博客内容。',
      });
    }
    ...
    try {
      ...
      const response = await aiClient.models.generateContent({
        ...
        contents,
        config: {
          abortSignal: AbortSignal.timeout(config.upstreamTimeoutMs),
          httpOptions: { timeout: config.upstreamTimeoutMs },
          systemInstruction: buildAgentSystemInstruction(snapshot.content),
          temperature: 0.7,
          candidateCount: 1,
          maxOutputTokens: config.maxOutputTokens,
        },
      });
      const upstreamReplyText = response.text?.trim();
      ...
      if (isTimeoutError(error)) {
        structuredLog('warn', 'gemini_timeout', req, res, metadata);
        return sendApiError(
          res,
          504,
          'AI_UPSTREAM_TIMEOUT',
      ...
      structuredLog('error', 'gemini_failure', req, res, metadata);
      return sendApiError(
        res,
        503,
        'AI_UNAVAILABLE',
