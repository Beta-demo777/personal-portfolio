...
test('returns the deterministic local response when Gemini is not configured', async () => {
  await withTestServer(async ({ origin }) => {
    ...
    assert.match(payload.message || '', /GEMINI_API_KEY/);
  });
});
...
test('uses a validated stale CMS snapshot for AI facts while excluding drafts', async () => {
  ...
  content.blogPosts.push({
    ...content.blogPosts[0],
    id: 'private-draft',
    title: 'PRIVATE_DRAFT_MUST_NOT_REACH_GEMINI',
    status: 'draft',
  });
  let systemInstruction = '';
  const fakeClient = {
    models: {
      generateContent: async (request: { config?: { systemInstruction?: string } }) => {
        systemInstruction = request.config?.systemInstruction || '';
        return { text: 'verified stale answer' };
      },
    },
  } as unknown as GoogleGenAI;
  const server = await startTestServer(
    { ...TEST_ENVIRONMENT, GEMINI_API_KEY: 'test-key' },
    {
  ...
  try {
    ...
    assert.doesNotMatch(systemInstruction, /PRIVATE_DRAFT_MUST_NOT_REACH_GEMINI/);
  } finally {
    await server.close();
  }
});

test('bounds Gemini replies so the next browser turn remains valid', async () => {
  const upstreamReply = `${'x'.repeat(1_498)}😀tail`;
  const fakeClient = {
    models: {
      generateContent: async () => ({ text: upstreamReply }),
    },
  } as unknown as GoogleGenAI;
  const server = await startTestServer(
    {
